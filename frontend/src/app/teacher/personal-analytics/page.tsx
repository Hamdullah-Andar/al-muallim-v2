...
export default async function StudentAnalytics() {
  ...
  const customAssignmentsList = assignments
    ...
    .map(a => {
      ...
      if (cat.includes('nawafil') || title.includes('nawafil') || title.includes('sunnah') || title.includes('ishraq') || title.includes('duha') || title.includes('tahajjud') || title.includes('maghrib')) icon = "ðŸŒ™";
      else if (cat.includes('quran') || cat.includes('read') || title.includes('quran') || title.includes('read') || title.includes('tafseer') || title.includes('book') || title.includes('riaz')) icon = "ðŸ“š";
      else if (cat.includes('exercise') || title.includes('exercise') || title.includes('walk') || title.includes('sport') || title.includes('gym') || title.includes('workout')) icon = "âš¡";
      else if (cat.includes('zikr') || title.includes('zikr') || title.includes('astaghfirullah') || title.includes('subhan')) icon = "ðŸ“¿";
  ...
  const displaySubjects = [
    { title: "Nawafil & Sunnahs", score: getScoreText(nawafilRate), icon: "ðŸŒ™", description: "Voluntary & night prayers" },
    { title: "Quran Recitation", score: getScoreText(quranRate), icon: "ðŸ“–", description: "Daily reading & reflection" },
    { title: "Guarding Senses", score: getScoreText(munkaratRate), icon: "ðŸ›¡ï¸", description: "Munkarat purity score" },
    { title: "Daily Adhkar", score: getScoreText(zikrRate), icon: "ðŸ“¿", description: "Remembrance & Duas" }
  ];
